...
function MineFilterInnhold(props: MineFilterInnholdProps) {
  ...
  const fjernUtilgjengeligeFilter = elem => {
    const arbeidsliste = elem.filterValg.ferdigfilterListe.includes(
      "MIN_ARBEIDSLISTE"
    );
    const arbeidslisteKategori =
      elem.filterValg.arbeidslisteKategori.length > 0;
    const veiledergrupper = elem.filterValg.veiledere.length > 0;
    const nyeBrukere = elem.filterValg.ferdigfilterListe.includes(
      "NYE_BRUKERE_FOR_VEILEDER"
    );
    const ufordelteBrukere = elem.filterValg.ferdigfilterListe.includes(
      "UFORDELTE_BRUKERE"
    );

    return !(
      (erPaEnhetensOversikt &&
        (arbeidsliste || arbeidslisteKategori || nyeBrukere)) ||
  ...
  const hentInnhold = () => {
    return filtrertListe().length > 0
      ? hentFiltrertListeinnhold()
      : getEmptyState();
  };
  ...
  return (
    <>
      <HiddenHjelpetekst
        type={
          useWindowWidth() < 1200
            ? PopoverOrientering.Venstre
            : PopoverOrientering.Over
        }
        hidden={filtrertListe().length === props.mineFilter.length}
        className={
          erRedesignFeatureTogglePa ? "ny__hjelpetekst" : "gammelt__hjelpetekst"
        }
      >
        {erPaMinOversikt &&
