...
function MineFilterRad({
  ...
  const veilederIdent = useSelector(
    (state: AppState) => state.inloggetVeileder.data!
  );
  const veilederIdentTilNonsens = mapVeilederIdentTilNonsens(
    veilederIdent.ident
  );
  ...
  function scrollAndSelect() {
    if (
      ...
      if (
        parentDiv.current.offsetTop + parentDiv.current.scrollTop >
          checkboxRef.current.offsetTop ||
        checkboxRef.current.offsetTop >
          parentDiv.current.offsetTop + parentDiv.current.clientHeight
      ) {
