...
import {
  ...
  handleDragOver,
  handleDragStart
} from "./mouse-drag-event-listeners";
...
function DragAndDropContainer({
  ...
  const alfabetiskSort = () => {
    dragAndDropOrder.sort((a: LagretFilter, b: LagretFilter) => {
      return a.filterNavn
        .toLowerCase()
        .localeCompare(b.filterNavn.toLowerCase(), undefined, {
          numeric: true
        });
    });
  ...
  const requestNewOrder = useCallback(
    (from: number, to: number) => {
      if (
        ...
        to !== -1 &&
        from !== -1 &&
        to < dragAndDropOrderRef.current.length
      ) {
        if (from < to)
          setAriaTekst(
  ...
  const eventIsInsideContainer = e =>
    dragContainer.current !== null && dragContainer.current.contains(e.target);

  useEventListener(
    "dragstart",
    handleDragStart({ eventIsInsideContainer, setSrcIndex, setDropIndex })
  );
  useEventListener(
    "dragenter",
    handleDragEnter({ eventIsInsideContainer, setdDragIsInsideElement })
  );
  useEventListener(
    "dragover",
    handleDragOver({ eventIsInsideContainer, setDestIndex })
  );
  useEventListener(
    "dragend",
    handleDragEnd({
