...
export function leggTilFerdigFilter(
  filterListe: string[],
  filter: string
): string[] {
  const newList = filter => filterListe.filter(v => v !== filter);
  if (
    filter === IKKE_PERMITTERTE_ETTER_NIENDE_MARS &&
    filterListe.includes(PERMITTERTE_ETTER_NIENDE_MARS)
  ) {
    return [...newList(PERMITTERTE_ETTER_NIENDE_MARS), filter];
  } else if (
    filter === PERMITTERTE_ETTER_NIENDE_MARS &&
    filterListe.includes(IKKE_PERMITTERTE_ETTER_NIENDE_MARS)
  ) {
    ...
    const checkboxFilter = filterListe.filter(valgtfilter =>
      CHECKBOX_FILTER.includes(valgtfilter)
    );
...
export function fjernFerdigfilter(
  valgtFilterList: string[],
  removeFilter: string
): string[] {
