...
function FilteringVeilederGrupper({filtergruppe}: FilteringVeilederGrupperProps) {
    ...
    const submitEndringer = (gruppeNavn: string, filterValg: FiltervalgModell) => {
        ...
        if (enhet) {
            dispatch(lageNyGruppe(endringer, enhet)).then(resp =>
                dispatch(endreFiltervalg('veiledere', resp.data.filterValg.veiledere, filtergruppe))
            );
        }
    };
    ...
    const veilederGrupperOK = () => {
        return lagretFilter.length > 0 ? (
            <VeilederGruppeInnhold veiledergruppe={sortertVeiledergruppe} filtergruppe={filtergruppe} />
        ) : (
