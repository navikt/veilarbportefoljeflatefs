...
export function LagredeFilterUIController(props: {
  ...
  const filtreringVeilederoversikt = useSelector(
    (state: AppState) => state.filtreringVeilederoversikt
  );

  const lagretMineFilter = useSelector(
    (state: AppState) => state.mineFilter.data
  );
  const lagretVeilederGrupper = useSelector(
    (state: AppState) => state.veiledergrupper.data
  );

  useEffect(() => {
    const getFiltrering = () => {
      if (props.filtergruppe === ListevisningType.veilederOversikt)
        return filtreringVeilederoversikt;
      else if (props.filtergruppe === ListevisningType.minOversikt)
        return filtreringMinoversikt;
      else if (props.filtergruppe === ListevisningType.enhetensOversikt)
        return filtreringEnhetensOversikt;
    };
    ...
    if (valgtMineFilter.length === 0) {
      ...
      logEvent(
        "portefolje.metrikker.lagredefilter.direkte-filtrering",
        {},
    ...
    if (valgtVeilederGruppe.length === 0) {
      ...
      dispatch(
        markerValgtVeilederGruppe(valgtVeilederGruppe[0], props.filtergruppe)
      );
    }
  }, [
    dispatch,
