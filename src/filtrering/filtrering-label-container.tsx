  ...
  const filterElementer = Object.entries(filtervalg)
    .map(([key, value]) => {
      if (key === "fodselsdagIMnd") {
            ...
            <FiltreringLabel
              ...
              label={
                key === "tiltakstyper"
                  ...
                  : singleValue.label ||
                    FilterKonstanter[key][singleValue] ||
                    singleValue
              }
              slettFilter={() =>
                slettEnkelt(key, singleValue.key || singleValue)
              }
            />
          );
        });
        ...
        muligMenIkkeValgt = harMuligMenIkkeValgtKolonne(
          listevisning,
          Kolonne.UTLOP_AKTIVITET
        );
        return Object.entries(value)
          .filter(
            ([_, aktivitetvalue]) => aktivitetvalue !== AktiviteterValg.NA
          )
          .map(([aktivitetkey, aktivitetvalue]) => (
            <FiltreringLabel
              ...
              harMuligMenIkkeValgtKolonne={
                muligMenIkkeValgt && aktivitetvalue === AktiviteterValg.JA
              }
            />
          ));
        ...
        kolonne =
          key === "ytelse" ? Kolonne.UTLOP_YTELSE : getKolonneFraLabel(value);
        muligMenIkkeValgt =
          kolonne === Kolonne.AVTALT_AKTIVITET &&
  ...
  actions: {
    ...
    slettEnkelt: (filterKey: string, filterValue: boolean | string | null) => {
      ...
      if (filterValue === "MIN_ARBEIDSLISTE") {
        dispatch(
          endreFiltervalg("arbeidslisteKategori", [], ownProps.filtergruppe)
        );
      }
    }
  }
