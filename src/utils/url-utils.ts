...
export function setFraBrukerIUrl(bruker: string) {
  ...
  if (lastSearch && fnrRegex.test(lastSearch)) {
    ...
    localStorage.setItem(
      "lastsearch",
      lastSearch.concat(`&fraBruker=${bruker}`)
    );
  }
...
export function getSideFromUrl() {
  return parseInt(
    (queryString.parse(window.location.search).side as string) || "1",
    10
  );
}
...
export function getSorteringsRekkefolgeFromUrl() {
  return (
    queryString.parse(window.location.search).sorteringsrekkefolge || IKKE_SATT
  );
}
