...
function lagPortefoljeForVeileder(queryParams, alleBrukere) {
  const enhetportefolje = lagPortefolje(
    queryParams,
    inloggetVeileder.enheter[0].enhetId,
    alleBrukere
  );
  ...
  const filtrerteBrukere = alleBrukere
    ...
    .map((bruker, index) => {
      if (index < 5) {
        ...
        bruker.oppfolgingStartdato = faker.date.between(
          new Date("2015-01-01"),
          new Date()
        );
...
mock.get(
  ...
  jsonResponse(customVeilederGrupper)
);
...
mock.delete(
  ...
  (req, res, ctx) => {
    ...
    if (!isNaN(filterId)) {
      customVeilederGrupper = customVeilederGrupper.filter(
        v => v.filterId !== filterId
      );
...
mock.get(
  ...
  jsonResponse(customMineFilter)
);

mock.put("/veilarbfilter/api/minelagredefilter/", ({ body }, res, ctx) => {
  let filterIndex = customMineFilter.findIndex(
    elem => elem.filterId === body.filterId
  );
...
mock.post(
  ...
  (req, res, ctx) => {
    ...
    sorteringer.forEach(elem => {
      const customMineFilterElem = customMineFilter.find(
        filter => elem.filterId === filter.filterId
      );
...
mock.get(
  ...
  delayed(1000, jsonResponse(statustall))
);
...
mock.post("/veilarboppfolging/api/tilordneveileder/", ({ body }, res, ctx) =>
  res(ctx.json(tildel(body)))
);
