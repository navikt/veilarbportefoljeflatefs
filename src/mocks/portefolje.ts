...
function lagGrunndata() {
  ...
  return {
    fnr: `${dag.toString().padStart(2, "0")}${mnd
      .toString()
      .padStart(2, "0")}${ar
      .toString()
    ...
    moteStartTid,
    moteSluttTid:
      moteStartTid && new Date(moteStartTid.getTime() + 15 * 60 * 1000)
  };
}
...
function lagYtelse() {
  ...
  if (ytelse === "AAP_MAXTID" || ytelse === "AAP_UNNTAK") {
    ...
    const aaptidUke = Math.round(
      (rndDate - todayDate) / (1000 * 60 * 60 * 24 * 7)
    );
...
function lagBruker(sikkerhetstiltak = [], egenAnsatt = false) {
  ...
  const veilederId =
    maybeVeileder < veiledere.length ? veiledere[maybeVeileder].ident : null;
