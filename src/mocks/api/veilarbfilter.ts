...
export const veilarbfilterHandlers: RequestHandler[] = [
    http.get('/veilarbfilter/api/enhet/:enhetId', async () => {
        return HttpResponse.json(customVeiledergrupper);
    }),
    http.put('/veilarbfilter/api/enhet/:enhetId', async ({request}) => {
    ...
    http.post('/veilarbfilter/api/enhet/:enhetId', async ({request}) => {
    ...
    http.delete('/veilarbfilter/api/enhet/:enhetId/filter/:filterId', async ({params}) => {
    ...
    http.get('/veilarbfilter/api/minelagredefilter', async () => {
        return HttpResponse.json(customMineFilter);
    }),
    http.put('/veilarbfilter/api/minelagredefilter', async ({request}) => {
        const oppdaterFilterRequest = (await request.json()) as LagretFilter;
    ...
    http.post('/veilarbfilter/api/minelagredefilter', async ({request}) => {
        const opprettFilterRequest = (await request.json()) as LagretFilter;
        const filterId = Math.floor(Math.random() * 100) + 500;
    ...
    http.delete('/veilarbfilter/api/minelagredefilter/:filterId', async ({params}) => {
        const filterId = parseInt(params.filterId as string);
    ...
    http.post('/veilarbfilter/api/minelagredefilter/lagresortering', async ({request}) => {
