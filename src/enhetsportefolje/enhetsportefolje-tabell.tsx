...
const finnBrukersVeileder = (veiledere, bruker) =>
...
function EnhetTabell(props: EnhetTabellProps) {
  ...
  const {
    ...
    listevisning,
    portefolje
  } = usePortefoljeSelector(ListevisningType.enhetensOversikt);
  const veiledere = useSelector((state: AppState) => state.veiledere);

  const dispatch = useDispatch();

  const settMarkert = (fnr, markert) =>
    dispatch(settBrukerSomMarkert(fnr, markert));

  const tilordningerStatus =
    portefolje.tilordningerstatus !== STATUS.RELOADING
      ? STATUS.OK
      : STATUS.RELOADING;
          ...
          <ul className="brukerliste">
            {brukere.map(bruker => (
              <EnhetBrukerpanel
                ...
                brukersVeileder={finnBrukersVeileder(
