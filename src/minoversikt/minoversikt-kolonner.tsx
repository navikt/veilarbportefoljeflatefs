...
function MinoversiktDatokolonner({className, bruker, filtervalg, valgteKolonner, enhetId, intl}: Props) {
    ...
    const moteStartTid = bruker.moteStartTid? new Date(bruker.moteStartTid) : null;
    const moteSluttTid = bruker.moteSluttTid ? new Date(bruker.moteSluttTid) : null;
    const varighet = moteSluttTid && moteStartTid ? new Date(moment(moteSluttTid).diff(moteStartTid)) : null;
