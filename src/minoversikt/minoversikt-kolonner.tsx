...
function MinoversiktDatokolonner({
  className,
  bruker,
  filtervalg,
  valgteKolonner,
  enhetId
}: MinOversiktKolonnerProps) {
  const { ytelse } = filtervalg;
  const ytelsevalgIntl = ytelsevalg();
  const erAapYtelse = Object.keys(ytelseAapSortering).includes(ytelse!);
  const valgteAktivitetstyper = utledValgteAktivitetsTyper(
    bruker.aktiviteter,
    filtervalg.aktiviteter
  );
  // TODO: bør gjøres før data lagres i storen
  const arbeidslisteFrist = bruker.arbeidsliste.frist
    ? new Date(bruker.arbeidsliste.frist)
    : null;
  const utlopsdatoUkerIgjen = utlopsdatoUker(bruker.utlopsdato);
  const venterPaSvarFraBruker = bruker.venterPaSvarFraBruker
    ? new Date(bruker.venterPaSvarFraBruker)
    : null;
  const venterPaSvarFraNAV = bruker.venterPaSvarFraNAV
    ? new Date(bruker.venterPaSvarFraNAV)
    : null;
  ...
  const nyesteUtlopteAktivitet = bruker.nyesteUtlopteAktivitet
    ? new Date(bruker.nyesteUtlopteAktivitet)
    : null;
  const ytelseErValgtKolonne = valgteKolonner.includes(Kolonne.UTLOP_YTELSE);
  const ytelseAapVedtaksperiodeErValgtKolonne = valgteKolonner.includes(
    Kolonne.VEDTAKSPERIODE
  );
  const ytelseAapRettighetsperiodeErValgtKolonne = valgteKolonner.includes(
    Kolonne.RETTIGHETSPERIODE
  );
  const ferdigfilterListe = !!filtervalg ? filtervalg.ferdigfilterListe : "";
  const rettighetsPeriode = aapRettighetsperiode(
    ytelse,
    bruker.aapmaxtidUke,
    bruker.aapUnntakUkerIgjen
  );
  ...
  return (
    <div className={className}>
      ...
      <UkeKolonne
        ...
        skalVises={
          ...
          (ytelse === ytelsevalgIntl.DAGPENGER ||
            ...
            ytelse ===
              ytelsevalgIntl.DAGPENGER_MED_PERMITTERING_FISKEINDUSTRI ||
      ...
      <UkeKolonne
        ...
        minVal={2}
        skalVises={
          ytelseErValgtKolonne && ytelse === ytelsevalgIntl.TILTAKSPENGER
        }
      />
      <DatoKolonne
        className="col col-xs-2"
      ...
      <DatoKolonne
        ...
        skalVises={
          !!ferdigfilterListe && ferdigfilterListe.includes(UTLOPTE_AKTIVITETER)
        }
      />
      <DatoKolonne
        className="col col-xs-2"
      ...
      <DatoKolonne
        className="col col-xs-2"
        dato={
          bruker.nesteAktivitetStart
            ? new Date(bruker.nesteAktivitetStart)
            : null
        }
        skalVises={
          !!ferdigfilterListe &&
      ...
      <DatoKolonne
        className="col col-xs-2"
        dato={
          bruker.forrigeAktivitetStart
            ? new Date(bruker.forrigeAktivitetStart)
            : null
        }
        skalVises={
          !!ferdigfilterListe &&
