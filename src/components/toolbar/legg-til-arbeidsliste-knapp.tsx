...
import {
  VIS_ARBEIDSLISTE_MODAL,
  visArbeidslisteModal
} from "../../ducks/modal";
import "./toolbar.less";
...
function LeggTilArbeidsliste(props: LeggTilArbeidslisteProps) {
  ...
  const modalSkalVises =
    useSelector((state: AppState) => state.modal.modal) ===
    VIS_ARBEIDSLISTE_MODAL;
  ...
  const valgteBrukere = portefolje.brukere.filter(
    bruker => bruker.markert === true
  );

  const skalSkjules =
    innloggetVeileder && pathname === "/portefolje"
      ? ident
...
function ArbeidsListeKnapp(props: {
  ...
  const inneholderBrukerMedArbeidsliste = props.valgteBrukere.some(
    bruker => bruker.arbeidsliste.arbeidslisteAktiv
  );
  const inneholderBrukerMedOgUtenArbeidsliste =
    inneholderBrukerMedArbeidsliste &&
    props.valgteBrukere.some(bruker => !bruker.arbeidsliste.arbeidslisteAktiv);
  ...
  const arbeidslisteButton = tekst => (
    <button
      ...
      data-testid={
        inneholderBrukerMedArbeidsliste
          ? "fjern-fra-arbeidsliste_knapp"
          : "legg-i-arbeidsliste_knapp"
      }
    >
      <ArbeidslisteIkonLinje
        className="toolbar-knapp__ikon"
        id="arbeidsliste-ikon"
      />
