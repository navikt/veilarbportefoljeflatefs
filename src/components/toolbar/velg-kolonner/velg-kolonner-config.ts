...
import {getFiltreringState} from '../../../ducks/ui/valgte-kolonner-selectors';
...
export function useAlternativerConfig(oversiktType: OversiktType) {
    const filtervalg = useSelector((s: AppState) => getFiltreringState(s, oversiktType));
    const kandidatForUtmelding = filtervalg[Filtervalg.ferdigfilterListe].includes(KANDIDAT_FOR_UTMELDING);

    return useMemo(() => {
        const alternativerConfig = new Map<Kolonne, Alternativ>();
        ...
        alternativerConfig.set(Kolonne.YTELSE_ARENA_VURDERINGSFRIST_AAP, {
            tekstlabel: 'Frist vurdering rett AAP (Arena)'
        });
        alternativerConfig.set(Kolonne.YTELSE_ARENA_YTELSESTYPE_AAP, {tekstlabel: 'Type AAP-periode (Arena)'});
        alternativerConfig.set(Kolonne.YTELSE_ARENA_VEDTAKSPERIODE_AAP, {
            tekstlabel: 'Gjenstående uker vedtak AAP (Arena)'
        });
        ...
        alternativerConfig.set(Kolonne.SISTE_ENDRING, {
            tekstlabel: 'Siste endring personen har gjort på aktivitet/mål'
        });
        ...
        alternativerConfig.set(Kolonne.ENSLIGE_FORSORGERE_AKIVITETSPLIKT, {
            tekstlabel: 'Om aktivitetsplikt overgangsstønad'
        });
        alternativerConfig.set(Kolonne.ENSLIGE_FORSORGERE_VEDTAKSPERIODE, {
            tekstlabel: 'Type vedtaksperiode overgangsstønad'
        });
        ...
        alternativerConfig.set(Kolonne.UTDANNING_OG_SITUASJON_SIST_ENDRET, {
            tekstlabel: 'Situasjon/utdanning sist endret'
        });
        ...
        alternativerConfig.set(Kolonne.FILTERHENDELSE_LENKE, {
            tekstlabel: kandidatForUtmelding ? 'Årsak til avslutning' : 'Hendelse (lenke)'
        });
        ...
        alternativerConfig.set(Kolonne.DAGPENGER_ANTALL_RESTERENDE_DAGER, {
            tekstlabel: 'Gjenstående dager dagpenger (DPSAK)'
        });
