...
import {
  selectMuligeAlternativer,
  selectValgteAlternativer
} from "../../../ducks/ui/listevisning-selectors";
...
function Listevisning(props: ListevisningProps) {
  const valgteAlternativ = useSelector((state: AppState) =>
    selectValgteAlternativer(state, props.filtergruppe)
  );
  const muligeAlternativer = useSelector((state: AppState) =>
    selectMuligeAlternativer(state, props.filtergruppe)
  );
  ...
  if (
    ![ListevisningType.minOversikt, ListevisningType.enhetensOversikt].includes(
      props.filtergruppe
    )
  ) {
    return null;
