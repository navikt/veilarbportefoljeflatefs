...
export function OppdaterMineFilter(props: {
  ...
  const [nyttFilterNavn, setNyttFilterNavn] = useState<string>(
    props.gammeltFilterNavn
  );

  const [feilmelding, setFeilmelding] = useState<LagretFilterValideringsError>(
    {} as LagretFilterValideringsError
  );
  ...
  const requestHandlerSlette = useRequestHandler(
    (state: AppState) => state.mineFilter.status,
    lukkModal
  );
  ...
  const doLagreEndringer = event => {
    ...
    const feilValideringResponse = feilValidering(
      trimmetFilterNavn,
      filterValg,
      data,
      filterId
