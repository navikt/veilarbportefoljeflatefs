...
export function lagretFilterValgModellErLik(
  filter1?: FiltervalgModell,
  filter2?: FiltervalgModell
): boolean {
...
function erValueTomt(value) {
  if (value instanceof Object) return erObjektValuesTomt(value);
    ...
    return (
      value === null || value === "NA" || value === "" || erTomtObjekt(value)
    );
}
...
export function feilValidering(
  filterNavn,
  filterValg,
  eksisterendeFilter,
  filterId?
  ...
  if (
    eksisterendeFilter.find(
      elem =>
        ...
        elem.filterNavn.toLowerCase() === filterNavn.toLowerCase()
    )
  ) {
  ...
  if (
    eksisterendeFilter.find(
      elem =>
        elem.filterId !== filterId &&
        lagretFilterValgModellErLik(elem.filterValg, filterValg)
    )
  ) {
