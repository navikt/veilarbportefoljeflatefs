...
export function MineFilterModal(props: { filtergruppe: string }) {
  ...
  const [valgtVisningstype, setValgtVisningstype] = useState<Visningstype>(
    Visningstype.MENY
  );
  ...
  useEffect(() => {
    if (filtreringMinOversikt.navnEllerFnrQuery.trim().length > 0)
      setValgtVisningstype(Visningstype.FNR_FEIL);
    else if (valgtMineFilter) setValgtVisningstype(Visningstype.OPPDATER);
    else if (!sisteValgtMineFilter)
      setValgtVisningstype(Visningstype.LAGRE_NYTT);
