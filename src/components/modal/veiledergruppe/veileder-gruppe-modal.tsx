...
export function VeilederGruppeModal(props: VeilederModalProps) {
  ...
  const [errors, setErrors] = useState<VeilederGruppeErrors>(
    {} as VeilederGruppeErrors
  );
  const [harForsoktSubmitte, setHarForsoktSubmitte] = useState(false);
  const [alertTekst, setAlertTekst] = useState("");

  const [visSletteVeiledergruppeModal, setSletteVeiledergruppeModal] = useState(
    false
  );
  const [visEndringerIkkeLagretModal, setEndringerIkkeLagretModal] = useState(
    false
  );
  ...
  function slettVeiledergruppeOgLukkModaler() {
    logEvent(
      "portefolje.metrikker.veiledergrupper.slettknapp",
      {},
  ...
  const lagredeGrupper = useSelector((state: AppState) =>
    state.veiledergrupper.data.filter(
      v => v.filterId !== props.initialVerdi.filterId
    )
  );
  ...
  useEffect(() => {
    if (
      lagredeGrupper.length > 0 &&
      erTomtObjekt(errors) &&
      props.isOpen &&
      props.initialVerdi.filterCleanup
    ) {
      const finnLikVeilederGruppe = lagredeGrupper.find(v =>
        veilederlisterErLik(
          v.filterValg.veiledere,
          props.initialVerdi.filterValg.veiledere
        )
      );
  ...
  const avbrytSletting = () => {
    logEvent(
      "portefolje.metrikker.veiledergrupper.avbrytknapp",
      {},
      { sideNavn: finnSideNavn() }
    );
    setSletteVeiledergruppeModal(false);
