...
function FjernFraArbeidslisteForm({
  ...
  const laster =
    slettFraArbeidslisteStatus !== undefined &&
    slettFraArbeidslisteStatus !== STATUS.OK;
  const className =
    valgteBrukere.length >= 22
      ? "arbeidsliste-listetekst__lang"
      : "arbeidsliste-listetekst";
  ...
  return (
    ...
    >
      <div className={className}>
        {visBrukerLabel && (
          <ul>{valgteBrukere.map(bruker => brukerLabel(bruker))}</ul>
        )}
      </div>
...
const mapDispatchToProps = dispatch => ({
  onSubmit: (valgteBrukere, lukkModal) => {
    const arbeidsliste: ArbeidslisteDataModell[] = valgteBrukere.map(
      bruker => ({
        fnr: bruker.fnr,
    ...
    slettArbeidsliste(arbeidsliste)(dispatch).then(res =>
