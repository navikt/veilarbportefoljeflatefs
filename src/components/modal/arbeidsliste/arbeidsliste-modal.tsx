...
class ArbeidslisteModal extends Component<
  ArbeidslisteModalProps,
  ArbeidslisteModalState
> {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      formIsDirty: false,
      laster:
        props.arbeidslisteStatus !== undefined &&
        props.arbeidslisteStatus !== STATUS.OK
    };
  ...
  fjernFraModal(valgteBrukere) {
    const brukereSomSkalFjernes = valgteBrukere.filter(
  ...
  render() {
    ...
    const fjerne = valgteBrukere.some(
      bruker => bruker.arbeidsliste.arbeidslisteAktiv
    );
