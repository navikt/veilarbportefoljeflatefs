...
export function endreSideBar({
  dispatch,
  currentListevisningsType,
  requestedTab
}: EndreSideBarProps) {
  dispatch({
...
function Sidebar(props: SidebarProps) {
  ...
  const isSidebarHidden = useSidebarViewStore(props.filtergruppe)
    .isSidebarHidden;
  ...
  function handleKeyUp(e, tab) {
    const sidebarTabs: NodeListOf<HTMLDivElement> = document.querySelectorAll(
      "button.sidebar__tab"
    );
    e.preventDefault();
    ...
    if (keyCode(e) === keyCodes.space) {
      ...
      sidebarTabs[tabFoc].setAttribute(
        "className",
        "sidebar__tab sidebar__tab-valgt"
      );
  ...
  function handleOnTabClicked(e, tab: Sidebar) {
    endreSideBar({
      ...
      currentListevisningsType: erPaMinOversikt
        ? ListevisningType.minOversikt
        : ListevisningType.enhetensOversikt
    });
  ...
  const Tabs = () => {
    const visVeiledergrupper = tab =>
      tab.type === SidebarTabType.VEILEDERGRUPPER;
    if (erPaMinOversikt) {
      return sidebar
        .filter(tab => !visVeiledergrupper(tab))
        .map((tab, key) =>
          mapTabTilView(tab, tab.type === selectedTabData.type, key)
        );
    }
    return sidebar.map((tab, key) =>
      mapTabTilView(tab, tab.type === selectedTabData.type, key)
    );
  };
  ...
  return (
    <div
      ...
      className={classNames(
        "sidebar",
        props.isSidebarHidden && "sidebar__hidden",
        "tabs"
      )}
    >
      <div
