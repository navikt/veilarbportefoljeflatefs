...
function VeilederCheckboxListe(props: any) {
  ...
  const erValgt = (value: string | undefined): boolean => {
    return (
      !!value && !!valgteElementer.find(valgtElement => value === valgtElement)
    );
  };
  ...
  const getFiltrerteVeiledere = (): VeilederModell[] => {
    ...
    const query = veilederNavnQuery
      ? veilederNavnQuery.toLowerCase().trim()
      : "";
  ...
  const handleCheckboxOnClick = (value: string | undefined) => {
    ...
    if (!valueErValgt) {
      ...
      valgteElem = valgteElementer.filter(
        valgtElement => value !== valgtElement
      );
      setValgteElementer(valgteElem);
    }
  };
  ...
  const mapVeiledereToCheckboxList = (
...
const mapDispatchToProps = (dispatch): DispatchProps => ({
  sokEtterVeileder(veiledere: string[]) {
    dispatch(
      endreFiltervalg("veiledere", veiledere, ListevisningType.veilederOversikt)
    );
  }
});
