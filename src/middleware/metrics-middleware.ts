...
function finnElementerSomErLagtTil(
  prevElementer: string[],
  nyeElementer: string[]
): string[] {
...
function finnSlettetGruppe(store: any, filterId: number) {
  const lagretGruppe = store
    .getState()
    .mineFilter.data.find(v => v.filterId === filterId);
  if (lagretGruppe) {
...
export const metricsMiddleWare = (store: any) => (next: any) => (
  action: any
) => {
  ...
  switch (type) {
    ...
    case SETT_SORTERING:
      loggEndreSortering(
        sideNavn,
        action.sorteringsfelt,
        action.sorteringsrekkefolge
      );
      break;
...
export const loggEndreMineFilter = (
  sideNavn: SideNavn,
  data: FilterEndringData,
  store: any
) => {
  const veilederIdent = mapVeilederIdentTilNonsens(
    store.getState().inloggetVeileder.data.ident
  );
  ...
  if (Array.isArray(data.filterVerdi)) {
    ...
    const lagtTilFilterVerdier = finnElementerSomErLagtTil(
      prevFilter,
      data.filterVerdi
    );
...
const loggEndreSortering = (
  ...
  if (
    ...
    logEvent("portefolje.metrikker.endre_sortering", {
      sideNavn,
      sorteringsfelt,
      rekkefolge
    });
...
const loggRedigerVeiledergruppeOK = (antallVeiledere, sideNavn: SideNavn) => {
  logEvent(
    "portefolje.metrikker.veiledergrupper.lagring-vellykket",
...
const loggSlettVeiledergruppeOK = (opprettetTidspunkt, sideNavn: SideNavn) => {
  logEvent(
    ...
    {
      levetid:
        (new Date().getTime() - new Date(opprettetTidspunkt).getTime()) /
        (1000 * 3600 * 24)
    },
    { sideNavn }
  );
...
const loggNyttMineFilterOK = (sideNavn: SideNavn) => {
  logEvent(
    "portefolje.metrikker.lagredefilter.oppretting-vellykket",
    { sideNavn: sideNavn },
    {}
  );
};
...
const loggRedigerMineFilterOK = (sideNavn: SideNavn) => {
  logEvent(
    "portefolje.metrikker.lagredefilter.lagring-vellykket",
    { sideNavn: sideNavn },
    {}
  );
};
...
const loggSorterMineFilterOK = (sideNavn: SideNavn) => {
  logEvent("portefolje.metrikker.lagredefilter.sortering-vellykket", {
    sideNavn: sideNavn
  });
};

const loggSorterMineFilterFeilet = (sideNavn: SideNavn) => {
  logEvent("portefolje.metrikker.lagredefilter.sortering-feilet", {
    sideNavn: sideNavn
  });
};
