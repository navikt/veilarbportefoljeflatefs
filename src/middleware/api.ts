...
export function tilordneVeileder(tilordninger) {
  ...
  const config = {
    ...MED_CREDENTIALS,
    method: "post",
    body: JSON.stringify(tilordninger)
  };
  return fetch(url, config).then(sjekkStatuskode);
}

export function redigerVeiledergruppe(
  endringer: RedigerLagretFilter,
  enhetId: string
): Promise<RedigerLagretFilter> {
  const url = `${VEILARBFILTER_URL}/enhet/${enhetId}`;
  const config = {
    ...MED_CREDENTIALS,
    method: "put",
    body: JSON.stringify(endringer)
  };
  return fetchToJson(url, config);
}

export function nyVeiledergruppe(
  endringer: NyttLagretFilter,
  enhetId: string
): Promise<NyttLagretFilter> {
  const url = `${VEILARBFILTER_URL}/enhet/${enhetId}`;
  const config = {
    ...MED_CREDENTIALS,
    method: "post",
...
export function slettVeiledergruppe(
...
export function httpArbeidsliste(arbeidsliste, method, additionalPath = "") {
  ...
  const config = {
    ...MED_CREDENTIALS,
    method,
    body: JSON.stringify(arbeidsliste)
  };
  return fetchToJson(url, config);
}
...
export function redigerMineFilter(
  endringer: RedigerLagretFilter
): Promise<RedigerLagretFilter> {
  const url = `${VEILARBFILTER_URL}/minelagredefilter/`;
  const config = {
...
export function nyttMineFilter(
  nyttFilter: NyttLagretFilter
): Promise<NyttLagretFilter> {
  const url = `${VEILARBFILTER_URL}/minelagredefilter/`;
  const config = {
    ...MED_CREDENTIALS,
    method: "post",
    body: JSON.stringify(nyttFilter)
  };
  return fetchToJson(url, config);
}
...
export function lagreSorteringFiltere(
  sorteringOgIder: SorteringOgId[]
): Promise<number> {
  const url = `${VEILARBFILTER_URL}/minelagredefilter/lagresortering/`;
  const config = {
    ...MED_CREDENTIALS,
    method: "post",
    body: JSON.stringify(sorteringOgIder)
  };
  return fetchToJson(url, config);
}
