...
export function selectMuligeAlternativer(
  state: AppState,
  name: string
): Kolonne[] {
...
export function selectValgteAlternativer(
  state: AppState,
  name: string
): Kolonne[] {
  if (name === ListevisningType.minOversikt) {
...
export function selectListeVisning(
  state: AppState,
  name: string
): ListevisningState {
  if (name === ListevisningType.minOversikt) {
...
function harValgtMinstEnAktivitet(
  aktiviteter?: FiltreringAktiviteterValg
): boolean {
  return (
    !!aktiviteter &&
    Object.entries(aktiviteter).filter(
      ([_, value]) => value === AktiviteterValg.JA
    ).length >= 1
  );
}
...
export function getFiltertingState(
  state: AppState,
...
export function getMuligeKolonner(
  ...
  return ([] as Kolonne[])
    .concat(
      addHvis(
        Kolonne.MOTER_IDAG,
        filtervalg.ferdigfilterListe.includes(MOTER_IDAG)
      )
    )
    .concat(
      addHvis(
        Kolonne.MOTER_VARIGHET,
        filtervalg.ferdigfilterListe.includes(MOTER_IDAG)
      )
    )
    .concat(
      addHvis(
        Kolonne.UTLOPTE_AKTIVITETER,
        filtervalg.ferdigfilterListe.includes(UTLOPTE_AKTIVITETER)
      )
    )
    .concat(
      addHvis(
        Kolonne.AVTALT_AKTIVITET,
    ...
    .concat(
      addHvis(
        Kolonne.VEDTAKSTATUS,
        filtervalg.ferdigfilterListe.includes(UNDER_VURDERING)
      )
    )
    .concat(
      addHvis(
        Kolonne.VEDTAKSTATUS_ENDRET,
        filtervalg.ferdigfilterListe.includes(UNDER_VURDERING)
      )
    )
    .concat(
    ...
    .concat(
      addHvis(
        ...
        name === ListevisningType.minOversikt &&
          filtervalg.ferdigfilterListe.includes(I_AVTALT_AKTIVITET)
      )
    )
    .concat(
    ...
    .concat(
      addHvis(Kolonne.VEILEDER, name === ListevisningType.enhetensOversikt)
    )
    .concat(
      addHvis(Kolonne.NAVIDENT, name === ListevisningType.enhetensOversikt)
    )
