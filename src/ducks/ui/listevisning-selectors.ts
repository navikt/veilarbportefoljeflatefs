...
export function getMuligeKolonner(state: AppState, name: ListevisningType): Kolonne[] {
    ...
    return [Kolonne.BRUKER, Kolonne.FODSELSNR]
        ...
        .concat(addHvis(Kolonne.VENTER_SVAR_FRA_NAV, filtervalg.ferdigfilterListe.includes(VENTER_PA_SVAR_FRA_NAV)))
        .concat(addHvis(Kolonne.VENTER_SVAR_FRA_BRUKER, filtervalg.ferdigfilterListe.includes(VENTER_PA_SVAR_FRA_BRUKER)))
        ...
        .concat(addHvis(Kolonne.ARBEIDSLISTE, name === ListevisningType.minOversikt && filtervalg.ferdigfilterListe.includes(MIN_ARBEIDSLISTE)));
}
