...
function AktivitetFilterform(props) {
    const fields = Object.entries(props.valg)
        .map(([kode, verdi]) => [
            <div key={`skjemaelement skjemaelement--horisontal aktivitet-${kode}`} className="aktivitetvalg blokk-xxs">
                <Field id={`aktivitet-${kode}-ja`} name={kode} value="JA" type="radio" className="skjemaelement__input radioknapp" component="input"/>
                <label htmlFor={`aktivitet-${kode}-ja`}><span className="sr-only">{verdi}</span></label>
                <Field id={`aktivitet-${kode}-nei`} name={kode} value="NEI" type="radio" className="skjemaelement__input radioknapp" component="input"/>
    ...
    return (
        <form className="skjema aktivitetfilterform" onSubmit={submithandler}>
            ...
            <button type="submit" className="knapp knapp--hoved knapp--mini">Lagre</button>
            <button type="button" className="knapp knapp--standard knapp--mini" onClick={reset(props.change, props.valg)}>Fjern valg</button>
        </form>
    );
}
