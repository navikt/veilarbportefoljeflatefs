describe('Gå inn til annen veileders oversikt via tabellen', () => {
    ...
    it('Velg Herman Thoresen', () => {
        cy.getByTestId('veilederoversikt_navn_lenke').contains('Thoresen, Herman').click();
        cy.getByTestId('annen-veileder_infotekst').should('be.visible')
...
describe('Søk veileder i veilederoversikt', () => {
    ...
    it('Søk på Glosli ', () => {
        ...
        cy.getByTestId('veilederoversikt_sok-veileder_veilederliste').children().should('have.length', 40)
        cy.getByTestId('veilederoversikt_sok-veileder-input').type('Gloslido');
        cy.getByTestId('veilederoversikt_alertstripe_info').should('contain', 'Ingen veiledere funnet')
            .should('be.visible');
        cy.getByTestId('veilederoversikt_sok-veileder-input').click().clear().type('Glosli');
        cy.getByTestId('veilederoversikt_sok-veileder_veilederliste').children().should('have.length', 1)
        cy.getByTestId('veilederoversikt_sok-veileder_lukk-knapp').contains('Lukk').should('be.visible');
        cy.getByTestId('veilederoversikt_sok-veileder_velg-knapp').should('not.be.visible');
    })
    it('Velg Glosli ', () => {
        cy.getByTestId('veilederoversikt_veilederliste_tbody').children().should('have.length', 20);
        ...
        cy.getByTestId('veilederoversikt_sok-veileder_velg-knapp').contains('Velg').should('be.visible').click();
        cy.getByTestId('veilederoversikt_veilederliste_tbody').children().should('have.length', 1);
        cy.getByTestId('filtreringlabel').contains('Glosli, Kasper').should('be.visible');
    })
})
