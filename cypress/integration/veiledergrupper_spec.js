...
describe("Lag ny veiledergruppe", () => {
  ...
  it("Det skal være riktig antall veiledergrupper ", () => {
    cy.getByTestId("veiledergruppe_rad-wrapper").should(
      "have.length",
      antallVeiledergrupper
    );
  });
  ...
  it("Søk veileder", () => {
    ...
    cy.getByTestId("veiledergruppe_modal_sok-veileder-input").should(
      "have.value",
      andersen
    );
    cy.getByTestId("veiledergruppe_modal_gruppenavn-input").should(
      "have.value",
      ""
    );
  });
  ...
  it("Validering: Tomt gruppenavn", () => {
    cy.getByTestId("veiledergruppe_modal_form").contains(
      "Gruppen mangler navn, legg inn gruppenavn."
    );
  });
  it("Validering: Eksisterende gruppenavn", () => {
    cy.getByTestId("veiledergruppe_modal_gruppenavn-input").type(
      eksisterendeGruppenavn
    );
    cy.getByTestId("veiledergruppe_modal_lagre-knapp").click();
    cy.getByTestId("veiledergruppe_modal_form").contains(
      "Gruppenavn er allerede i bruk."
    );
  });
...
describe("Rediger filtervalg", () => {
  it("Klikk på blyantsymbolet", () => {
    cy.getByTestId(
      `rediger-veiledergruppe_knapp_${gruppenavnRedigert}`
    ).click();
  });
  it("Fjern veiledere", () => {
  ...
  it('Antall veiledere skal være 0, og det skal stå "Ingen veiledere lagt til i gruppen"', () => {
    cy.getByTestId("veiledergruppe_modal_antall-valgte-veiledere_0").should(
      "exist"
    );
