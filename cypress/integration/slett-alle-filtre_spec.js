describe('Slett alle filtre', () => {
    ...
    it('Velg to statusfiltre', () => {
        ...
        cy.getByTestId('filter_checkboks-container_inaktiveBrukere').check({force: true})
    ...
    it('Velg to tiltakstyper', () => {
        ...
        cy.getByTestId('checkbox-filterform_velg-knapp').should('be.enabled').click();
    })
    it('Klikk på slett alle filtervalg-etikett', () => {
        cy.getByTestId('filtreringlabel').should('have.length', 7);
        cy.getByTestId('filtreringlabel').contains("Slett alle filtervalg").click();
